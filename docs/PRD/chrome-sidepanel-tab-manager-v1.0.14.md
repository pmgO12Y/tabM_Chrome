# Chrome 侧边栏标签管理插件 PRD v1.0.14

## 1. Executive Summary
开发一款运行在 Chrome 侧边栏中的标签管理插件，面向 300+ 标签页的重度用户场景，提供按窗口浏览、快速搜索、切换、关闭和固定等基础能力。首版以高性能、低内存、模块化扩展为核心，优先解决“浏览器标签过多导致定位和切换效率下降”的问题，并进一步修正普通网页 favicon 与 Chrome 顶部标签栏的显示差异。

## 2. Problem Statement
- 多窗口、多任务并行的 Chrome 重度用户，需要在大量标签中快速定位目标页面。
- 原生横向标签栏难以同时表达窗口上下文和标签层级，切换成本高。
- 当前实现会在归一化阶段把普通网页 favicon 直接替换为 `_favicon` 代理地址，导致某些网站在侧边栏中显示的图标与 Chrome 顶部标签栏不同。
...
- 对普通网页标签，图标一致性应优先贴近 Chrome 当前标签对象里的原始 favicon，而不是优先走代理。
...
- 普通网页标签改为“原始 favicon 优先、代理兜底”。

### 网页图标展示约束
- 普通 `http/https/file` 标签在归一化阶段保留原始 `tab.favIconUrl`。
- 侧边栏渲染阶段按 `raw favIconUrl -> Chrome _favicon -> 首字母` 顺序尝试。
- 非网页标签仍保持简单安全策略，不引入额外图标实验逻辑。
...
- 普通网页标签在侧边栏中显示的 favicon，应优先与 Chrome 顶部标签栏当前图标一致。

## 7. User Stories / Acceptance Criteria
- 作为用户，我可以按窗口查看标签，以理解当前工作上下文。
- 作为用户，我可以搜索标题或 URL，以更快定位目标标签。
- 作为用户，我可以直接切换、关闭或固定一个标签，而不必回到顶部标签栏。
- 作为用户，普通网页标签在侧边栏中应优先显示与顶部标签一致的原始 favicon，而不是代理出的替代图标。
- 验收标准：
  - `https`、`http`、`file` 标签按 `raw favIconUrl -> _favicon -> 首字母` 顺序显示。
  - 普通网页 favicon 在归一化阶段不再被强行替换成 `_favicon`。
  - 标题和 URL 文本不因该策略发生变化。
...
- 直接使用网页原始 favicon URL 会比仅使用 `_favicon` 更贴近顶部标签栏，但也意味着侧边栏页会直接请求该 favicon 资源。

## 10. Open Questions
- 后续是否需要给“原始 favicon 加载失败”的网站做更明显的占位样式。
- 是否需要针对极少数返回异常 favicon 的网站再加站点级降级规则。

## 11. Verification Plan
- 自动验证：
  - 单元测试覆盖网页原始 favicon 保留、网页 favicon 候选顺序、非网页协议不走代理。
  - 类型检查确保归一化和渲染链路保持兼容。
  - 构建产物必须可生成 `background` 与 `sidepanel` 两个入口。
- 手工验证：
  - 验证像 Yahoo 这类站点，侧边栏图标优先与顶部标签栏一致。
  - 验证原始 favicon 失败时仍可回退到 `_favicon`。
  - 验证标题和 URL 文本保持不变。
