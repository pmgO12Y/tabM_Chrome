# Chrome 侧边栏标签管理插件 PRD v1.0.8

## 1. Executive Summary
开发一款运行在 Chrome 侧边栏中的标签管理插件，面向 300+ 标签页的重度用户场景，提供按窗口浏览、快速搜索、切换、关闭和固定等基础能力。首版以高性能、低内存、模块化扩展为核心，优先解决“浏览器标签过多导致定位和切换效率下降”的问题，并持续收敛侧边栏中网页图标与 Chrome 标签栏之间的认知偏差。

## 2. Problem Statement
- 多窗口、多任务并行的 Chrome 重度用户，需要在大量标签中快速定位目标页面。
- 原生横向标签栏难以同时表达窗口上下文和标签层级，切换成本高。
- 当标签被休眠类插件替换为 `chrome-extension://...` 页面时，侧边栏可能显示插件页图标而不是原网页图标，影响扫描和识别效率。
...
- 对休眠插件替换页执行“原网页 favicon 优先，插件页 favicon 降级”的保守还原策略。

### 图标展示约束
- 普通 `http/https/file` 标签继续优先使用 Chrome `_favicon` 代理。
- `chrome-extension://...` 标签如果能从查询参数或 hash 参数中解析出原网页地址，则优先显示原网页 favicon。
- 若解析失败，则保留安全的扩展页 favicon；若扩展页 favicon 也不可用，则回退为标题首字母。
...
- 常见休眠插件替换页在能提供原网页地址时，侧边栏图标应恢复为原网页 favicon。

## 7. User Stories / Acceptance Criteria
- 作为用户，我可以按窗口查看标签，以理解当前工作上下文。
- 作为用户，我可以搜索标题或 URL，以更快定位目标标签。
- 作为用户，我可以直接切换、关闭或固定一个标签，而不必回到顶部标签栏。
- 作为用户，当标签被休眠插件替换后，只要扩展页里保留了原网页地址，我仍能通过原网页 favicon 识别它。
- 验收标准：
  - `https`、`http`、`file` 标签继续显示对应网页 favicon。
  - 休眠插件替换页存在常见 `url/uri/original/page/target` 参数时，应优先还原原网页 favicon。
  - 休眠插件替换页无法还原原网页时，应降级为插件页 favicon 或首字母，而不是空白。
  - 标题和 URL 文本不因该策略发生变化。
...
- 不同休眠插件的扩展页参数结构不统一，通用策略只能覆盖常见模式，不能承诺 100% 还原原网页 favicon。

## 10. Open Questions
- 后续是否需要为具体休眠插件增加定向解析规则，提高还原率。
- 是否需要在 UI 中增加“已休眠标签”弱提示，帮助用户理解图标来源。

## 11. Verification Plan
- 自动验证：
  - 单元测试覆盖普通网页 favicon 代理、查询参数休眠页还原、hash 参数休眠页还原、解析失败后的扩展页 favicon 降级。
...
- 手工验证：
  ...
  - 验证休眠插件替换页存在原网页参数时，侧边栏图标恢复原网页 favicon。
  - 验证休眠插件替换页不存在原网页参数时，侧边栏图标降级为插件页 favicon 或首字母。
  - 验证标题和 URL 文本保持不变。
