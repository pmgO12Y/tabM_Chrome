...
- 若扩展本身存在加载等待、视觉噪声、热区不一致或卡顿，会削弱标签管理工具的价值。
...
- 标签行的整块可见 hover 热区必须与实际可点击激活热区一致，不允许出现“外层高亮但不可点击”的假热区。
...
- 作为用户，鼠标移入标签行时，看到的 hover 区域就是可点击激活的区域。
- 验收标准：
  - 搜索命中结果不得被折叠态隐藏。
  - 当前激活标签必须有清晰高亮。
...
- 手工验证：
  - 验证 hover 区域与整行点击热区一致。
  - 验证点击行主体会激活标签，点击右侧按钮不会误激活整行。
  - 验证当前激活标签变化时，侧边栏会自动滚动定位。
