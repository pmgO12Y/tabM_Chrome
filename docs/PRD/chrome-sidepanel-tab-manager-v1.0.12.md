# Chrome 侧边栏标签管理插件 PRD v1.0.12

## 1. Executive Summary
开发一款运行在 Chrome 侧边栏中的标签管理插件，面向 300+ 标签页的重度用户场景，提供按窗口浏览、快速搜索、切换、关闭和固定等基础能力。首版以高性能、低内存、模块化扩展为核心，优先解决“浏览器标签过多导致定位和切换效率下降”的问题，并继续提升扩展页标签的图标一致性。

## 2. Problem Statement
- 多窗口、多任务并行的 Chrome 重度用户，需要在大量标签中快速定位目标页面。
- 原生横向标签栏难以同时表达窗口上下文和标签层级，切换成本高。
- 对 `chrome-extension://...` 标签，`management.get()` 虽能返回扩展声明的 icon URL，但这些资源未必对当前扩展页面可访问；结果是顶部标签已有真实 icon，侧边栏仍可能退回首字母。
...
- 对扩展页图标，优先调用 Chrome 自己的 favicon 管道，比直接加载其他扩展的资源 URL 更稳定。
...
- 为扩展页标签提供基于 `chrome://favicon/chrome-extension://扩展ID/` 的更强图标候选源。

### 图标展示约束
- 普通 `http/https/file` 标签继续优先使用 Chrome `_favicon` 代理。
- `chrome-extension://...` 标签优先尝试 Chrome 内部扩展 favicon URL：`chrome://favicon/size/16@2x/chrome-extension://扩展ID/`。
- 若该内部 favicon 不可用，再回退到 `management` 读取到的扩展 icon URL。
- 再失败时，继续按 `_favicon -> favIconUrl -> 首字母` 降级。
...
- 常见扩展页标签在侧边栏中优先显示 Chrome 自己可识别的扩展 favicon，而不是退回首字母。

## 7. User Stories / Acceptance Criteria
- 作为用户，我可以按窗口查看标签，以理解当前工作上下文。
- 作为用户，我可以搜索标题或 URL，以更快定位目标标签。
- 作为用户，我可以直接切换、关闭或固定一个标签，而不必回到顶部标签栏。
- 作为用户，当标签本身是某个扩展页时，侧边栏应优先尝试 Chrome 内部 favicon 通道，而不是依赖可能被拦截的第三方扩展图片 URL。
- 验收标准：
  - `chrome-extension://...` 标签会先尝试 `chrome://favicon/size/16@2x/chrome-extension://扩展ID/`。
  - 若该候选成功，侧边栏优先显示该图标。
  - 若该候选失败，再回退到 `management` icon、`_favicon`、`favIconUrl`、首字母。
  - 标题和 URL 文本不因该策略发生变化。
...
- 智能分类、规则引擎或 AI 功能。
- 账号同步与跨设备同步。
- 对其他扩展未公开资源做额外抓取或绕过访问限制。

## 9. Risks / Dependencies
- 依赖 Chrome Side Panel API、Manifest V3 service worker、`chrome.tabs`、`chrome.storage`、`chrome.favicon` 和 `chrome.management`。
- Chrome 对内部 `chrome://favicon` 通道的行为属于浏览器实现细节，不同版本可能存在差异，但它通常比直接加载其他扩展资源 URL 更接近顶部标签栏的实际结果。

## 10. Open Questions
- 如果内部 `chrome://favicon` 对个别扩展页仍不可用，是否要在 UI 上弱提示“该扩展未暴露可访问图标”。
- 是否需要把成功拿到的扩展页 favicon 做持久缓存，减少频繁重试。

## 11. Verification Plan
- 自动验证：
  - 单元测试覆盖扩展 ID 提取、内部扩展 favicon URL 生成、候选顺序、候选去重。
  - 类型检查确保 favicon 候选逻辑和扩展 icon 缓存逻辑保持兼容。
  - 构建产物必须可生成 `background` 与 `sidepanel` 两个入口。
- 手工验证：
  - 验证普通网页标签的图标与 Chrome 标签栏一致。
  - 验证此前显示首字母的扩展页标签，优先尝试内部扩展 favicon 通道。
  - 验证当其他扩展资源不可直接访问时，侧边栏仍可通过内部 favicon 通道显示更接近顶部的图标。
  - 验证标题和 URL 文本保持不变。
