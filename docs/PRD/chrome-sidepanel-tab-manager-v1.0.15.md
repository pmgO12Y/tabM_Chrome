# Chrome 侧边栏标签管理插件 PRD v1.0.15

## 1. Executive Summary
开发一款运行在 Chrome 侧边栏中的标签管理插件，面向 300+ 标签页的重度用户场景，提供按窗口浏览、快速搜索、切换、关闭和固定等基础能力。首版以高性能、低内存、模块化扩展为核心，优先解决“浏览器标签过多导致定位和切换效率下降”的问题，并为 The Marvellous Suspender 一类休眠标签恢复原网页显示信息。

## 2. Problem Statement
- 多窗口、多任务并行的 Chrome 重度用户，需要在大量标签中快速定位目标页面。
- 原生横向标签栏难以同时表达窗口上下文和标签层级，切换成本高。
- 休眠插件会把原网页替换为 `chrome-extension://.../suspended.html` 页面，导致侧边栏只看到插件页标题、URL 和图标，而不是原网页信息。
...
- 对休眠标签，应优先恢复“显示层”的原网页信息，而不改变真实标签操作行为。
...
- 对 `suspended.html` 这类休眠页提取原网页标题、URL 和 favicon。

### 休眠标签展示约束
- 仅在 `chrome-extension://...` 且路径包含 `suspended` 时尝试解析。
- 先从 query / hash 中提取原网页 URL，优先键包括 `uri`、`url`、`original`、`page`、`target`。
- 标题优先提取 `ttl`、`title`。
- 成功解析原网页 URL 后：
  - 侧边栏显示的 `title` 改为原网页标题。
  - 侧边栏显示的 `url` 改为原网页 URL。
  - 侧边栏 favicon 改为原网页 `_favicon`。
- 若解析失败，则保留当前插件页标题、URL 和原有 favicon 降级逻辑。
- 该策略只改变显示层，不改变 `tabId`、激活、关闭、固定等真实操作对象。
...
- 常见 suspended URL 结构的休眠标签在侧边栏中应恢复为原网页标题、URL 和 favicon。

## 7. User Stories / Acceptance Criteria
- 作为用户，我可以按窗口查看标签，以理解当前工作上下文。
- 作为用户，我可以搜索标题或 URL，以更快定位目标标签。
- 作为用户，我可以直接切换、关闭或固定一个标签，而不必回到顶部标签栏。
- 作为用户，当标签被休眠插件替换后，侧边栏应尽量显示原网页信息，而不是只显示休眠插件页。
- 验收标准：
  - `suspended.html?ttl=...&url=...` 结构可恢复原网页标题和 URL。
  - `suspended.html#ttl=...&uri=...` 结构可恢复原网页标题和 URL。
  - 恢复成功后，favicon 使用原网页 `_favicon`。
  - 恢复失败时，保持原插件页信息，不出现空白或错误跳转。
...
- 智能分类、规则引擎或 AI 功能。
- 账号同步与跨设备同步。
- 针对每个休眠插件做高度定制的私有协议适配。

## 9. Risks / Dependencies
- 依赖 Chrome Side Panel API、Manifest V3 service worker、`chrome.tabs`、`chrome.storage` 和 `chrome.favicon`。
- 不同休眠插件的参数命名可能不同，本轮只覆盖常见的 `ttl/title` 与 `uri/url/...` 结构。

## 10. Open Questions
- 后续是否要扩大支持更多 suspended URL 参数命名。
- 是否要在 UI 中弱提示“该标签为休眠恢复显示”。

## 11. Verification Plan
- 自动验证：
  - 单元测试覆盖 query 形式和 hash 形式的 suspended URL 解析。
  - 单元测试覆盖恢复失败时保留原插件页信息。
  - 类型检查确保归一化逻辑与现有状态结构兼容。
  - 构建产物必须可生成 `background` 与 `sidepanel` 两个入口。
- 手工验证：
  - 验证 The Marvellous Suspender 一类休眠标签在侧边栏中显示原网页标题和 URL。
  - 验证休眠标签 favicon 恢复为原网页图标。
  - 验证点击该项仍然只激活当前休眠标签，而不是新开网页。
